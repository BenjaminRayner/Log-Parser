...
fn view_double_and_triple_dicts_concurrent(double_dict:&Arc<DashMap<String, i32>>, triple_dict:&Arc<DashMap<String, i32>>) {
    packages::parser::print_dict_concurrent("double", double_dict);
    packages::parser::print_dict_concurrent("triple", triple_dict);
}
...
fn main() {
    ...
    if let Some(true) = args.single_map
    {   // seperate map implementation
        let (double_dict, triple_dict, _all_token_list) = packages::parser::parse_raw_seperate(input_fn.unwrap(), &log_format, args.num_threads.unwrap() as usize);
    ...
    { // copy pasted code...for handling concurrent implementation
        let (double_dict, triple_dict, _all_token_list) = packages::parser::parse_raw_concurrent(input_fn.unwrap(), &log_format, args.num_threads.unwrap() as usize);

        view_double_and_triple_dicts_concurrent(&double_dict, &triple_dict);
        ...
        for triple in extended_sample_string_tokens.windows(3) {
            let three_gram = format!("{}^{}^{}", triple[0], triple[1], triple[2]);
            if triple_dict.contains_key(&three_gram) && triple_dict.get(&three_gram).unwrap().value() < &cutoff {
                // println!("3-gram {}, count {}", three_gram, &triple_dict.get(&three_gram).unwrap());
                uncommon_3grams.push(three_gram);
            }
        }
        ...
        for two_g in deduped_2grams_from_uncommon_3grams {
            let two_g_count = double_dict.get(&two_g).unwrap();
            println!("2-gram {}, count {}", two_g, two_g_count.value());
            if two_g_count.value() < &cutoff {
                uncommon_2grams.push(two_g);
                // println!("2-gram {}, count {}", two_g, two_g_count);
            }
        }
